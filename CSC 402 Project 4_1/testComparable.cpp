...
int main()
{
	...
	If Integer, Character, and Double have been declared explicit, the only way to construct a Comparable<>
	using one of those types would be to first explicitly construct the enclosed type:

	Comparable<Integer> { Integer(10) };

	Finally, what about these two? Should they work or not?  Not according to
	  the standard, so we will avoid them.
	...
	*/
	...
	// You can see if these work or not in your compiler
	...
	// We can avoid the need for a named Character object if desired
	Comparable<Character> cc2{ Character{'Z' } };
	cout << "cc1 = " << cc1 << endl;
	cout << "cc2 = " << cc2 << endl;
